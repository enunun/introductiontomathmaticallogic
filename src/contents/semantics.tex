...
\index[widx]{はいりほう@背理法}
\begin{Lemma}[背理法] \label[Lemma]{lemma:semanticcontradiction}
	\(\Gamma\)を閉論理式の集合，\(\varphi\)を論理式とする．
	このとき
	\[
		\Gamma \satisfy \varphi \metaequivalent \Gamma \satisfy \lnot \varphi \to \bot
	\]
	が成り立つ．
\end{Lemma}
...
\index[widx]{ぜんしょうじょきょりつ@全称除去律}
\begin{Lemma}[全称除去律] \label[Lemma]{lemma:semanticforallelimination}
	\(\varphi, \psi\)を論理式，\(x\)を変数記号，\(a\)を\(\varphi\)中の\(x\)に代入可能な項とする．
	このとき，任意の閉論理式の集合\(\Gamma\)に対して
	\[
		\Gamma \satisfy \forall x \varphi \metaimplies \Gamma \satisfy \subst{\varphi}{a/x}
	\]
	が成り立つ．
\end{Lemma}
...
\index[widx]{そんざいじょきょりつ@存在除去律}
\begin{Lemma}[存在除去律] \label[Lemma]{lemma:semanticexistselimination}
	\(\Gamma, \Delta\)を閉論理式の集合とし，
	\(\varphi, \psi\)を論理式，\(x\)を変数記号とする．
	このとき，
	\(\Gamma \satisfy \exists x \varphi\)
	かつ\(\varphi\)中の\(x\)に代入可能であって
	\(\apply{\Var}{a}\)の元が
	\(\exists x \varphi\)と\(\psi\)のいずれにも自由出現しないような任意の項\(a\)に対して
	\(\Delta \satisfy \subst{\varphi}{a/x} \to \psi\)
	が成り立つならば，
	\(\Gamma \cup \Delta \satisfy \psi\)
	が成り立つ．
\end{Lemma}
...
\index[widx]{ぜんしょうどうにゅうりつ@全称導入律}
\begin{Lemma}[全称導入律] \label[Lemma]{lemma:semanticforallinsertion}
	\(\Gamma\)を閉論理式の集合とし，\(x\)を変数記号，\(\varphi\)を論理式とする．
	\(\varphi\)中の\(x\)に代入可能であって\(\apply{\Var}{a}\)の元が
	\(\varphi\)に自由出現しないような任意の項\(a\)に対して
	\(	\Gamma \satisfy \subst{\varphi}{a/x}\)
	が成り立つとする．このとき\(\Gamma \satisfy \forall x \varphi\)
	が成り立つ．
\end{Lemma}
...
\index[widx]{そんざいどうにゅうりつ@存在導入律}
\begin{Lemma}[存在導入律] \label[Lemma]{lemma:semanticexistsinsertion}
	\(\Gamma\)を閉論理式の集合とし，\(x\)を変数記号，\(\varphi\)を論理式とする．
	このとき，\(\varphi\)中の\(x\)に代入可能な任意の項\(a\)に対して
	\[
		\Gamma \satisfy \subst{\varphi}{a/x} \metaimplies \Gamma \satisfy \exists x \varphi
	\]
	が成り立つ．
\end{Lemma}
...
\begin{proof}
	...
	\(\Gamma \satisfy \exists x \varphi\)となる．
\end{proof}

全称除去律と存在除去律は「\(\forall, \exists\)を含む論理式が論理的帰結として得られた場合にいえること」を表現したもので，
...
\(\forall, \exists\)を陽に含むものについては上記補題を使うことで確かめることができる．

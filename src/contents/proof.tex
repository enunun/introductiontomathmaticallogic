...
\begin{Def}[論理規則その2] \label[Def]{Def:quantiferrule}
	論理式の有限列\(\Gamma, \Delta\)と論理式\(\varphi, \psi\)および変数記号\(x\)と項\(a\)に対して，
	...
	\begin{multicols}{2}
		...
		\end{prooftree}
		ただし，\(\apply{\Var}{a}\)の元は\(\Gamma\)中の各論理式や\(\varphi\)に自由出現しない．
		\columnbreak
		\begin{prooftree}
	...
	\begin{multicols}{2}
		...
		\begin{prooftree}
			...
			\BinaryInfC{\(\Gamma, \Delta \sequent \psi\)}
		\end{prooftree}
		ただし，\(\apply{\Var}{a}\)の元は\(\Gamma, \Delta\)中の各論理式と\(\exists x \varphi, \psi\)のいずれにも自由出現しない．
	\end{multicols}
\end{Def}

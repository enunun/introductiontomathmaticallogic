...
\Cref{Def:term}で特徴的なのは，「項とは〇〇であるものをいう」などのような直接的な定義ではなく
「変数記号と定数記号からどのようにして項全体が得られるか」という構成方法を述べていることである．
何が項で何が項でないかが明確に定まっているため，この定義は定義として正当なものである．
\Cref{Def:term}で行ったような「ベースとなる対象から定義したい対象がどのように構成されるかを述べる」
形式でなされる定義を%
...
\begin{Ex} \label[Ex]{Ex:nologicalexpression}
	...
	また，群論の言語\(\symcal{L}_1\)において，\(\paren{\forall x \paren{\obj{x} \obj{\ast} \obj{e} = \obj{x}}}\)や
...
\begin{Note} \label[Note]{Note:logicalexpression}
	...
	\begin{itemize}
		...
		\item 論理記号\(\lnot, \land, \lor, \to\)たちの結合の優先順位については，\(\lnot\)がもっとも高く，次に\(\land, \lor\)が高く，
		      もっとも低いのが\(\to\)であると約束する．
		\item アリティ2の関数記号や関係記号については，\(\obj{x} \obj{\ast} \obj{y}\)や\(\obj{x} \le \obj{y}\)のように
		      その記号が真ん中に来るように配置して表記してよい．
	\end{itemize}
	こうすると，\(\paren{\paren{\varphi \land \paren{\lnot \psi}} \to \rho} \equiv \varphi \land \lnot \psi \to \rho\)のように見やすくできる%
	\footnote{%
...
なお，正確に書くとそれなりに面倒な議論が長くなるため，以下では相当にラフに記述していることに注意されたい．
...
\begin{Def} \label[Def]{Def:occurence}
	...
	\index[widx]{こう@項，\(\symcal{L}\)項!へいこう@閉---}%
	\term{閉項}であるという．
\end{Def}
...
\begin{Ex} \label[Ex]{Ex:substitution}
	...
	\begin{equation*}
		\subst{\varphi}{\obj{e} / x} \equiv
		\paren{\forall \obj{a} \paren{\forall y \paren{

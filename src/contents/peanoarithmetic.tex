...
\begin{proof}
	...
	\]
	となる．ゆえに\cref{item:PAA9}から\(\symcal{M} \satisfy \forall \obj{x} \apply{\varphi}{\obj{x}}\)を得る．従って完全性定理によって\(\PA \provable \text{\cref{item:PAA10}}\)となる．
\end{proof}
...
\begin{proof}
	\(\symcal{M}\)を\(\Robinson\)の任意のモデルとする．完全性定理により，\(\symcal{M}\)が右辺の論理式を充足することを示せば十分である．

	まずは\cref{eq:Robinsonnumeraltheoremaddition}を示そう．
	自然数\(n\)を任意に1つとったとき，
	...
	\cref{item:PAA4}から\(\symcal{M}\)においては
	\begin{align*}
		\numeral{n} \obj{+} \numeral{m + 1} & \objeq \numeral{n} \obj{+} \paren{\numeral{m} \obj{+} \obj{1}} \\
		                                    & \objeq \paren{\numeral{n} \obj{+} \numeral{m}} \obj{+} \obj{1} \\
		                                    & \objeq \numeral{n + \paren{m + 1}}
	...
	以上より，数学的帰納法によって任意の自然数\(n, m\)に対して\(\symcal{M} \satisfy \lnot \paren{\numeral{n} \objeq \numeral{m}}\)が成り立つ．
\end{proof}

...
こうして定義される\(\symcal{L}_{\Ar}\)構造\(\symcal{L}\)は\(\RArithmetic\)であって，
\[
...
一意性は，\(n \neq m\)となる任意の自然数\(n, m\)に対して\(\standardmodel \satisfy \lnot \paren{\numeral{n} \objeq \numeral{m}}\)が成り立つことから従う．

存在性は閉項\(t\)の構成に関する帰納法によって示す．
\(t\)が定数記号\(\obj{0}, \obj{1}\)のいずれかであれば，対応する自然数\(n\)として\(0, 1\)をそれぞれとればよい．
\(t\)が\(t_1 \obj{+} t_2\)の形のとき，帰納法の仮定から\(\standardmodel \satisfy t_1 \objeq \numeral{n_1}\)と
\(\standardmodel \satisfy t_2 \objeq \numeral{n_2}\)を満たす自然数\(n_1, n_2\)がとれるので，

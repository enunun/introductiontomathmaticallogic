...
\(\prec'\)が整礎であることを示そう．
\(L'\)を\(L\)の空でない部分集合とする．このとき，\(\Natural\)の部分集合
\begin{equation*}
	N = \Set{\apply{\depth}{t} | t \in L'}
\end{equation*}
は空でない．よって，\(\Natural\)上の通常の大小関係\(<\)の整礎性から\(N\)の極小元\(x\)をとることができる．
...
\]
とする．
このとき，\cref{Thm:inductivedifinition}により，写像\({\size} \colon L \to \Natural\)
で任意の\(t \in L\)に対して
\[

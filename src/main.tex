...
%
% 定理・補題・系：tcolorboxのtitle機能（enhanced skin）で，本文幅いっぱいの
% 黒帯ラベルを枠の上辺・左右にぴったり接して描く（すきま無し）．
% amsthmの\newtheoremが用意したカウンタ（定義と共有）・crefタイプ名は
% そのまま使い，環境の描画部分だけをtcolorboxに手組みで差し替える
% （\begin{Thm}[名前] \label[Thm]{...} ... という呼び出し方は不変）．
\tcbset{
...
% 「太さ＝重要度」で節は太く（3pt），小節は細く（2pt，グレー）．
% titlesec等の見出し専用パッケージには頼らず，\@startsectionが内部で呼ぶ
% \@sect・\@ssectと同じ流儀でそのまま手組みする．
% （\section*/\subsection*も\@ifstarで分岐して両対応する．白黒印刷前提のため
%   色はxcolorの黒・グレーのみを使う．）
\makeatletter
...
%\includeonly{contents/formalize,contents/computable,contents/answer}

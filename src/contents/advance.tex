...
\begin{Thm} \label[Thm]{Thm:HornHerbrandmodel}
	...
	理論\(\Gamma, \Gamma'\)が\(\Gamma \subset \Gamma'\)を満たすとする．
	\(\Gamma\)が無矛盾な全称Horn理論であるならば，
...
\begin{Ex} \label[Ex]{Ex:generatorforgrouptheory}
	...
	\end{align*}
	よく知られているように\(\GP\)はモデルをもつから無矛盾であり，
	等式理論であるから全称Horn理論でもある．
	よって，\cref{Thm:HornHerbrandmodel}から
	\(\symcal{L}\)における\(\Gamma\)のHerbrand構造\(\symcal{M}\)が\(\GP\)のモデルであることがわかる．
...
\begin{Ex} \label[Ex]{Ex:polynomialringherbrandmodel}
	...
	\(\GP\)と同様に\(\Ring\)も無矛盾な全称Horn理論であるから，
	\cref{Thm:HornHerbrandmodel}によって
	\(\polynomial{\symcal{R}}{x_1, x_2, \dots, x_n}\)が\(\Ring\)のモデルであることがわかる．
\end{Ex}

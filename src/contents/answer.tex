...
\Cref{Thm:MenkinTheorem}により，\(\Gamma \cup \Set{\lnot \varphi}\)のモデル\(\symcal{M}\)が存在する．
この\(\symcal{M}\)は\(\symcal{M} \satisfy \lnot \varphi\)を満たすので\(\symcal{M} \satisfy \varphi\)
は成り立たず，従って\(\Gamma \satisfy \varphi\)ではない．


\subsection*{\Cref{Que:Merbrandstructurewelldefineded}}
...
定義された\(\symcal{L}\)の各記号の\(\symcal{M}\)による解釈が，その代表元のとりかたによらないことを示す．

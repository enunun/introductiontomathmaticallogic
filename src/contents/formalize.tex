...
\begin{Que} \label{Que:recursivedefinition}
	...
	\begin{enumerate}
		...
		\item \(L_{n+1}\)は\(L_n \subset L_{n+1}\)と上記2を満たす包含関係に関して最小の集合である．
	\end{enumerate}
...
\begin{Que} \label[Que]{Que:recursivedefinitiondepth}
	\Cref{Que:well-foundedexample}における数式の複雑さを表す写像\(\depth \colon L \to \Natural\)は，以下の条件を満たすことを証明せよ．
	\begin{align}
	...
	なお，写像\(d\)は\cref{eq:Que:recursivedefinitiondepth_numeral}と
...
\begin{Que} \label{Que:inductivedifinition}
	\NewDocumentCommand{\size}{}{\mathop{\symup{size}}}
...
\begin{Que} \label{Que:structuralinduction}
	\NewDocumentCommand{\size}{}{\mathop{\symup{size}}}
	任意の数式\(t\)に対し，

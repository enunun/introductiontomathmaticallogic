...
\begin{Que} \label{Que:grouptheoryaxiomize}
	...
	上記のような例が存在することは，\(T\)が\(\symcal{L}_1\)に記号を追加して
	言語を拡張することによって\(T_1\)から自然に得られる理論だとはみなせないことを意味している．
\end{Que}
...
\begin{Lemma} \label{Lemma:equivrelationforisomorphic}
	\(\symcal{L}\)を言語とし，\(\symcal{M_1}, \symcal{M_2}, \symcal{M_3}\)を\(\symcal{L}\)構造とする．
	このとき
	\begin{align}
		\symcal{M_1}                          & \isomorphic \symcal{M_1},
		\label{eq:reflexivityforisomorphic}                                                         \\
		\symcal{M_1} \isomorphic \symcal{M_2} & \metaimplies \symcal{M_2} \isomorphic \symcal{M_1},
		\label{eq:symmetryforisomorphic}                                                            \\
		\text{\(\symcal{M_1} \isomorphic \symcal{M_2}\)かつ\(\symcal{M_2} \isomorphic \symcal{M_3}\)}
		                                      & \metaimplies \symcal{M_1} \isomorphic \symcal{M_3}
		\label{eq:transitivityforisomorphic}
	\end{align}
	が成り立つ．
\end{Lemma}

\begin{proof}
	\Cref{eq:reflexivityforisomorphic}は恒等写像が同型写像であることから，
	\cref{eq:symmetryforisomorphic}は同型写像の逆写像もまた同型写像であることから，
	\cref{eq:transitivityforisomorphic}は同型写像の合成もまた同型写像であることからそれぞれ従う．
\end{proof}
...
\Cref{Lemma:equivrelationforisomorphic}から，以下の補題が即座に得られる．

\begin{Lemma} \label{Lemma:equivrelationforelementarilyequivalent}
	\(\symcal{L}\)を言語とし，\(\symcal{M_1}, \symcal{M_2}, \symcal{M_3}\)を\(\symcal{L}\)構造とする．
	このとき
	\begin{align}
		\symcal{M_1}                                      & \elementarilyequivalent \symcal{M_1},
		\label{eq:reflexivityforelementarilyequivalent}                                                                    \\
		\symcal{M_1} \elementarilyequivalent \symcal{M_2} & \metaimplies \symcal{M_2} \isomorphic \symcal{M_1},
		\label{eq:symmetryforelementarilyequivalent}                                                                       \\
		\text{\(\symcal{M_1} \elementarilyequivalent \symcal{M_2}\)かつ\(\symcal{M_2} \elementarilyequivalent \symcal{M_3}\)}
		                                                  & \metaimplies \symcal{M_1} \elementarilyequivalent \symcal{M_3}
		\label{eq:transitivityforelementarilyequivalent}
	\end{align}
	が成り立つ．
\end{Lemma}
...
\begin{Thm} \label{eq:isomorphicforlogicalexpression}

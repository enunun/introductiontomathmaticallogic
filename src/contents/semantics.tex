...
この充足関係に着目したとき，閉論理式は以下の3パターンに分類することができる：
\begin{enumerate}
	\item あらゆる構造が充足する閉論理式，
	\item ある構造は充足するが，別の構造は充足しない閉論理式，
	\item どんな構造も充足しない閉論理式．
\end{enumerate}
...
\begin{Def} \label[Def]{Def:validformula}
	...
	閉論理式\(\varphi\)が恒偽式であることは，\(\lnot \varphi\)が恒真式であることと同値である．
\end{Def}

\begin{Ex} \label[Ex]{Ex:semanticlawofexcludedmiddle}
	\(\varphi\)を閉論理式とするとき，閉論理式\(\varphi \lor \lnot \varphi\)は恒真式である．
	...
	閉論理式\(\forall \obj{x} \lnot \paren{\obj{x} \obj{<} \obj{x}}\)を\(\varphi\)とするとき，\(\varphi\)は恒真式ではない．
...
\begin{Que} \label{Que:semanticlawofexcludedmiddle}
	\Cref{Ex:semanticlawofexcludedmiddle}の後半で述べた事実を検証せよ．すなわち，
	\(\symcal{L}_{\OrderedRing}\)構造\(\symcal{M}\)で閉論理式\(\forall \obj{x} \lnot \paren{\obj{x} \obj{<} \obj{x}}\)を
	充足しないものを挙げよ．
\end{Que}
...
\begin{Def} \label{Def:equivalentformula}
	...
	\]
	の全称閉包が恒真式であるとき，\(\varphi\)と\(\psi\)は\term{論理的に同値}であるといい
	\begin{equation}

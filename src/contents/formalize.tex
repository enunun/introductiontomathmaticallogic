...
\begin{Def} \label{Def:cantorpairfunctionn}
	...
	\begin{align}
		\begin{aligned}
			...
			\quad \text{(\(x \in \NaturalNumbers\))},                                                                                             \\
			\apply{\pairfunc^{n + 1}}{x_0, x_2, \dots, x_{n  - 1}, x_n} & = \apply{\pairfunc}{x_0, \apply{\pairfunc^n}{x_1, \dots, x_{n-1}, x_n}} \\
			                                                            & \quad \text{(\(x_0, x_2, \dots, x_{n-1}, x_n \in \NaturalNumbers\))}.
		\end{aligned}
...
\begin{Lemma} \label[Lemma]{lemma:cantorpairfunctionbijectionn}
	正の整数\(n\)に対し，\(\pairfunc^n\)は全単射であり，その逆写像
	\[
...
\begin{Lemma} \label[Lemma]{lemma:codizeforfinitesequencebijection}
	...
	\end{equation*}
	を対応させる\(\cup_{n \in \NaturalNumbers} \NaturalNumbers^{\Set{0,1,\dots,n-1}}\)から\(\NaturalNumbers\)への関数は全単射である．
\end{Lemma}

...
\begin{Def} \label{Def:semanticimplies}
	...
	\end{equation}
	と定める．
	さらに，\(\symcal{L}\)構造\(\symcal{M}\)と\(\symcal{L}\)論理式\(\varphi\)について，
	\(\symcal{M} \satisfy \varphi\)が成り立たないことを%
	\index[sidx]{\(\symcal{M} \notsatisfy \varphi\)：充足しない}
	\begin{equation}
		\symcal{M} \notsatisfy \varphi
		\label{eq:notsatisfy}
	\end{equation}
	...
	\(\symcal{M} \satisfy \varphi\)であるとき，\(\varphi\)は\(\symcal{M}\)において成り立つなどということもある．
\end{Def}
...
\begin{Ex} \label[Ex]{Ex:satisfy}
	\(\symcal{L}_{\GP}\)を\cref{Ex:grouptheory}で述べた群論の言語とする．
	整数全体の集合\(\Integers\)に対し，\(\Integers\)を対象領域とする\(\symcal{L}_{\GP}\)構造\(\symcal{M}\)を以下によって定義する：
	\begin{enumerate}
	...
	この\(\symcal{L}_{\GP}\)構造\(\symcal{M}\)は\cref{Ex:grouptheory}で挙げた3つの\(\symcal{L}_{\GP}\)文をすべて充足する．
	一方で，\(\symcal{M}\)による解釈のうち\(\symcal{M}\)による\(\obj{e}\)の解釈を\(\interpretation{\symcal{M}}{\obj{e}} = 1\)
	と置き換えて得られる\(\symcal{L}_{\GP}\)構造\(\symcal{M}'\)は，
	\cref{Ex:grouptheory}で挙げた3つの\(\symcal{L}_{|GP}\)文のうち1つ目のみを充足する．
\end{Ex}
...
\begin{Ex} \label[Ex]{Ex:model}
	\Cref{Ex:grouptheory}で述べた3つの\(\symcal{L}_{\GP}\)文からなる\(\symcal{L}_{\GP}\)理論\(\GP\)について考える．
	このとき，\cref{Ex:satisfy}で述べた\(\symcal{L}_{\GP}\)構造\(\symcal{M}\)は\(\GP\)のモデルである．
	一方で，\cref{Ex:satisfy}で述べた\(\symcal{L}_{\GP}\)構造\(\symcal{M}'\)は\(\GP\)のモデルではない．
\end{Ex}
...
\begin{Que} \label{Que:model}
	\Cref{Ex:grouptheory}で挙げた群の理論\(\GP\)について，次の論理式を\(\varphi\)とする：
	\[
...
\index[sidx]{\(\apply{\Th}{\symcal{M}}\)：構造から得られる理論}
...
\index[widx]{だいあぐらむ@ダイアグラム!しょとうだいあぐらむ@初等---}
\index[widx]{だいあぐらむ@ダイアグラム}
\index[sidx]{\(\apply{\eDiag}{\symcal{M}}\)：初等ダイアグラム}
\index[sidx]{\(\apply{\Diag}{\symcal{M}}\)：ダイアグラム}
\begin{Def} \label{Def:modeldiagram}
	\(\symcal{L}\)を言語とし，\(\symcal{M}\)を\(\symcal{L}\)構造とする．
	\(\symcal{M}\)を\(\languagewithname{\symcal{L}}{\symcal{M}}\)構造とみなしたときの
	\(\apply{\Th}{\symcal{M}}\)のことを
	\(\symcal{M}\)の\term{初等ダイアグラム}といい，
	\begin{equation}
...
\begin{Ex} \label[Ex]{Ex:isomorphic}
	\Cref{Ex:grouptheory}で挙げた群の理論\(\GP\)の2つのモデル\(\symcal{M}, \symcal{N}\)が同型であることは，
	...
	よって「群として同型」であるような2つの群は，\(\symcal{L}_{\GP}\)閉論理式で表現できる性質に差異はない．
	例えば「可換群である」という主張は\(\symcal{L}_{\GP}\)閉論理式として表現できる（\cref{Que:model}を参照）ので，
	同型な2つの群はどちらも可換であるか，どちらも可換でないかのいずれかである．
\end{Ex}

\begin{Note}
	\Cref{Thm:isomorphicforlogicalexpression}は「同型ならば初等同値」であることを示すが，
	逆の「初等同値ならば同型」は成り立たないことが知られている．
\end{Note}
...
\begin{Thm} \label[Thm]{Thm:semanticdne}
	\(\varphi\)を論理式とする．このとき，
	\begin{equation}
...
\begin{Lemma} \label[Lemma]{lemma:logicallyequivalentcharactarize}
	\(\varphi, \psi\)を論理式とする．このとき，\(\varphi, \psi\)が論理的に同値であるための必要十分条件は，
	任意の構造\(\symcal{M}\)に対して
	\begin{equation}
		\symcal{M} \satisfy \varphi \metaequivalent \symcal{M} \satisfy \psi
		\label{eq:logicallyequivalentcharactarize}
	\end{equation}
	が成り立つことである．
\end{Lemma}
...
\begin{Que} \label{Que:logicallyequivalentcharactarize}
	\Cref{lemma:logicallyequivalentcharactarize}において，\(\varphi, \psi\)が閉論理式とは限らない場合の証明を完成させよ．
\end{Que}


以上の準備のもと，よく知られた同値な論理式を以下に挙げておく．
\(\forall, \exists\)を陽に含まないものについては真理値表を書くことにより確かめることができる．
\(\forall, \exists\)を陽に含むものについては愚直に論理的同値性を示す必要がある．
この際，\cref{lemma:logicallyequivalentcharactarize}を使うとよい．


\begin{Thm} \label[Thm]{Thm:semantictodisjunctivenormalform}
...
\begin{Thm}[De Morganの法則] \label[Thm]{Thm:semanticDemorganslawproposition}
	...
	\begin{align}
		...
		\lnot \forall x \varphi          & \logicallyequivalent \exists x \lnot \varphi,
		\label{eq:semanticdemorganforall}                                                       \\
		\lnot \exists x \varphi          & \logicallyequivalent \forall x \lnot \varphi
		\label{eq:semanticdemorganexists}
	\end{align}
	が成り立つ．
\end{Thm}

\begin{proof}
	\Cref{eq:semanticdemorganforall}と\cref{eq:semanticdemorganexists}については証明しておこう．
	\(\symcal{M}\)を任意の構造とし，その対象領域を\(M\)とする．
	\Cref{lemma:logicallyequivalentcharactarize}の証明と同様に，
	...
	\Cref{eq:semanticdemorganexists}を示そう．
...
\begin{Thm} \label[Thm]{Thm:semanticcommunitativelaw}
	\(\varphi, \psi\)を論理式とする．
	このとき
	\begin{align}
...
\begin{Thm} \label[Thm]{Thm:semanticassociativelaw}
	\(\varphi, \psi\)を論理式とする．
	このとき
	\begin{align}
...
\begin{Thm} \label[Thm]{Thm:semantictolnotland}
	\(\varphi, \psi\)を論理式とする．
	このとき
	\begin{align}
		\lnot \paren{\varphi \to \psi}           & \logicallyequivalent \varphi \land \lnot \psi,
		\label{eq:semantictolnotland}                                                                                     \\
		\lnot \forall x \paren{\varphi \to \psi} & \logicallyequivalent \exists x \lnot \paren{\varphi \land \lnot \psi},
		\label{eq:semanticforallcontradiction}                                                                            \\
...
\begin{Thm} \label[Thm]{Thm:semanticforallexiststo}
	\(\varphi, \psi\)を論理式，\(x\)を変数記号とする．
	このとき
	\begin{align}
		\forall x \paren{\varphi \to \psi} & \logicallyequivalent \exists x \varphi \to \psi \quad \text{ただし\(x \notin \apply{\FV}{\psi}\)},
		\label{eq:semanticforalltoexists}                                                                                                    \\
		\exists x \paren{\varphi \to \psi} & \logicallyequivalent \forall x \varphi \to \psi \quad \text{ただし\(x \notin \apply{\FV}{\psi}\)}
		\label{eq:semanticexiststoforall}
	\end{align}
	が成り立つ．
\end{Thm}

\begin{Thm} \label[Thm]{Thm:semanticforallexistslandlor}
	\(\varphi, \psi\)を論理式，\(x\)を変数記号とする．
	このとき
	\begin{align}
		\forall x \paren{\varphi \land \psi} & \logicallyequivalent \forall x \varphi \land \forall x \psi,
		\label{eq:semanticforallland}                                                                       \\
		\exists x \paren{\varphi \lor \psi}  & \logicallyequivalent \exists x \varphi \lor \exists x \psi
		\label{eq:semanticexistslor}
	\end{align}
	が成り立つ．
\end{Thm}

\begin{Thm} \label[Thm]{Thm:semanticforallexistslandloroneside}
	\(\varphi, \psi\)を論理式，\(x\)を変数記号とする．
	このとき
	\begin{align}
		\forall x \paren{\varphi \lor \psi}  & \logicallyequivalent \forall x \varphi \lor \psi \quad \text{ただし\(x \notin \apply{\FV}{\psi}\)},
		\label{eq:semanticforallloroneside}                                                                                                     \\
		\exists x \paren{\varphi \land \psi} & \logicallyequivalent \exists x \varphi \land \psi \quad \text{ただし\(x \notin \apply{\FV}{\psi}\)}
		\label{eq:semanticexistslandoneside}
	\end{align}
	が成り立つ．
\end{Thm}

\index[widx]{\(\alpha\)どうちせい@\(\alpha\)同値性}
\begin{Thm}[\(\alpha\)同値性] \label[Thm]{Thm:alphaequivalent}
	\(\varphi\)を論理式，\(x, y\)を変数記号とする．\(\varphi\)中の\(x\)に\(y\)が代入可能であるならば
	\begin{align}
		\forall x \varphi & \logicallyequivalent \forall y \subst{\varphi}{y/x},
		\label{eq:alphaequivalentforall}                                         \\
		\exists x \varphi & \logicallyequivalent \exists y \subst{\varphi}{y/x}
		\label{eq:alphaequivalentexists}
	\end{align}
	が成り立つ．
\end{Thm}

\begin{Note}
	\Cref{Thm:alphaequivalent}は，
...
\begin{Ex} \label[Ex]{Ex:expressionsubstitution}
	\(\varphi, \psi\)を論理式とする．\cref{Thm:semantictolnotland}の\cref{eq:semantictolnotland}は真理値表を書くことによっても確かめられるが，
	\cref{Thm:semanticdne}の\cref{eq:semanticdne}や
	\cref{Thm:semantictodisjunctivenormalform}の\cref{eq:semantictodisjunctivenormalform}，
	および\cref{Thm:semanticDemorganslawproposition}の\cref{eq:semanticDemorganslawpropositionlor}を用いて以下のように導くこともできる：
	\begin{align*}
		\lnot \paren{\varphi \to \psi} & \logicallyequivalent  \lnot \paren{\lnot \varphi \lor \psi} \\
		                               & \logicallyequivalent  \lnot \lnot \varphi \land \lnot \psi  \\
		                               & \logicallyequivalent  \varphi \land \lnot \psi.
	\end{align*}
\end{Ex}

ただし，\cref{Ex:expressionsubstitution}では以下の性質を暗黙的に利用した．

\begin{Thm} \label[Thm]{Thm:logicallyequivalentequivalentrelation}
	\(\varphi_1, \varphi_2, \varphi_3\)を論理式とする．このとき，以下が成り立つ：
	\begin{enumerate}
		\item \(\varphi_1 \logicallyequivalent \varphi_1\),
		\item \(\varphi_1 \logicallyequivalent \varphi_2 \metaimplies \varphi_2 \logicallyequivalent \varphi_1\),
		\item \(\text{\(\varphi_1 \logicallyequivalent \varphi_2\)かつ\(\varphi_2 \logicallyequivalent \varphi_3 \metaimplies \varphi_1 \logicallyequivalent \varphi_3\)}\).
	\end{enumerate}
\end{Thm}

\begin{Que} \label{Que:expressionsubstitution}
	\Cref{Ex:expressionsubstitution}と同様にして，
	\cref{eq:semanticdemorganforall}や\cref{eq:semantictolnotland}を用いて
	\cref{eq:semanticforallcontradiction}を導け．
\end{Que}